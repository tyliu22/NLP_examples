...
def _get_train_data_loader(batch_size, training_dir, is_distributed):
    ...
    # padding or truncate the sentence to makse sure the sentence have same and fixed length
    input_ids = []
    attention_masks = []
    ...
    # class_counts = [num_non_disaster, num_disaster]
    # class_weights = 1. / torch.tensor(class_counts, dtype = torch.float)
    # sample_weights = [class_weights[label] for label in train_data.train_labels.value()]

    # # weighted sampler for imbalanced dataset
    # sampler = WeightedRandomSampler(sample_weights, num_samples=len(sample_weights), replacement=True)

    # Random sampler OR weighted sampler for imbalanced data
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=batch_size)

    return train_dataloader


def _get_test_data_loader(test_batch_size, testing_dir):
    dataset = pd.read_csv(os.path.join(testing_dir, "test.csv"))
    sentences = dataset.sentence.values
    labels = dataset.label.values
    ...
    # Convert the lists into tensors
    test_inputs = torch.cat(input_ids, dim=0)
    test_masks = torch.cat(attention_masks, dim=0)
...
def train(args):
    ...
    train_loader = _get_train_data_loader(args.batch_size, args.training_data_dir, is_distributed)
    ...
    # Only classfier layers are trained
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            model.zero_grad()  # difference between the model.zero_grad() and optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_labels",      type=int,   default=2,    metavar="N", help="Number of labels, binary classfication default value is 2")
    parser.add_argument("--batch-size",      type=int,   default=64,   metavar="N", help="batch size for training")
    parser.add_argument("--test-batch-size", type=int,   default=64, metavar="N", help="input batch size for testing")
    ...
    parser.add_argument("--training-data-dir",   type=str,  default="../dataset/", help="position of training dataset")
    parser.add_argument("--testing-data-dir",   type=str,  default="../dataset/", help="position of testing dataset")
    parser.add_argument("--num-gpus",            type=int,  default=0,             help="Number of GPUS")

    # Tokenizer --> DataLoader (test&train) --> model & optimizer & scheduler -->  model evaluation
